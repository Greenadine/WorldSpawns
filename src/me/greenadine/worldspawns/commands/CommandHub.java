...
public class CommandHub implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String args[]) {
        if (!(sender instanceof Player)) {
                ...
                if (target == null) {
                    ...
                    if (main.getConfig().getBoolean("settings.teleportDelay")) {
                        ...
                        if (world == null || xs == null || ys == null || zs == null || yaws == null || pitchs == null) {
                            ...
                            ScheduleHubTP hub = new ScheduleHubTP(main, target,
                                    main.getConfig().getInt("settings.teleportDelayHub"), x, y, z);
            ...
            if (!p.hasPermission((new Permissions()).hub)) {
            ...
            if (args.length == 0) {
                if (settings.getHub().getString("hub.world") == null) {
                    ...
                    if (main.getConfig().getBoolean("settings.teleportDelay")) {
                        ...
                        ScheduleHubTP hub = new ScheduleHubTP(main, p,
                                main.getConfig().getInt("settings.teleportDelayHub"), x, y, z);
                        hub.setTaskId(Bukkit.getScheduler().scheduleSyncRepeatingTask(main, hub, 0, 20));
                    } else {
                        World w = main.getServer().getWorld(settings.getHub().getString("hub.world"));
            ...
            if (args.length == 1) {
                if (!p.hasPermission((new Permissions()).hub_players)) {
                    p.sendMessage(noperm);
                    return true;
                }
                Player target = main.getServer().getPlayer(args[0]);
                if (target == null) {
                    ...
                    if (target == p) {
                        ...
                        if (main.getConfig().getBoolean("settings.teleportDelay")) {
                            ...
                            String yaws = settings.getHub().getString("hub.yaw");
                            String pitchs = settings.getHub().getString("hub.pitch");
                            if (world == null || xs == null || ys == null || zs == null || yaws == null
                                    || pitchs == null) {
                                p.sendMessage(prefix + ChatColor.translateAlternateColorCodes('&',
                                        Lang.COMMAND_HUB_NULL.toString()));
                                return true;
                            } else {
                                double x = target.getLocation().getX();
                                double y = target.getLocation().getY();
                                double z = target.getLocation().getZ();
                                ScheduleHubTP hub = new ScheduleHubTP(main, p, target,
                                        main.getConfig().getInt("settings.teleportDelayHub"), x, y, z);
                                hub.setTaskId(Bukkit.getScheduler().scheduleSyncRepeatingTask(main, hub, 0, 20));
                            }
                        } else {
